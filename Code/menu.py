...
class Menu:
    def __init__(self, display, input_handler):
        ...
        # Main navigation (added Grades)
        self.main_menu_items = ["Schedule", "Grades", "Clock", "Settings", "Set Time"]
    ...
    def scan_wifi_networks(self):
        ...
        try:
            ...
            try:
                # Use terse output and explicit fields to preserve SSIDs with spaces
    ...
    def _parse_nmcli_output(self, output):
        """Parse nmcli '-t -f SSID,SECURITY device wifi list' output.
        Lines are 'SSID:SECURITY' with SSID possibly empty for hidden networks.
        """
        networks = []
        for raw in output.strip().split('\n'):
            if not raw:
                continue
            # Split only on the first ':' to preserve ':' in SECURITY if any
            if ':' in raw:
                ssid, security = raw.split(':', 1)
            else:
                ssid, security = raw, ''
            ssid = ssid.strip()
            security = security.strip()
            if not ssid:
                # Skip hidden SSIDs in menu list
                continue
            is_open = (security == '' or security == '--' or security.lower() == 'none')
    ...
    def _load_canvas_config(self):
        """Load Canvas base URL and API token from config JSON.
        Searches in Code/canvas_config.json and local canvas_config.json.
        """
        candidates = [
            os.path.join(os.path.dirname(__file__), 'canvas_config.json'),
            os.path.join(os.getcwd(), 'canvas_config.json')
        ]
    ...
    def _fetch_canvas_courses(self):
        """Fetch courses and current scores fresh from Canvas (no cache)."""
        base_url, token = self._load_canvas_config()
        if not base_url or not token:
            self.display.show_message("Grades", "Canvas not\nconfigured", (255, 150, 100))
            time.sleep(1.5)
            return []
        try:
            url = base_url.rstrip('/') + '/api/v1/users/self/courses'
            params = {
                'include[]': 'enrollments',
                'per_page': 100
            }
            ...
            if resp.status_code != 200:
                self.display.show_message("Grades", f"HTTP {resp.status_code}", (255, 100, 100))
                time.sleep(1.5)
                return []
            courses = resp.json()
            items = []
            for c in courses:
                ...
                if enrollments:
                    ...
                    if score is None:
                        score = e.get('current_score')
                if score is None:
                    items.append(f"{name} : N/A")
                else:
                    try:
                        items.append(f"{name} : {round(float(score), 1)}%")
                    except Exception:
            ...
            self.display.show_message("Grades", f"Error: {str(e)[:28]}", (255, 100, 100))
            time.sleep(1.5)
            return []

    def show_grades_screen(self):
        """Build and display the grades list, fetching fresh each time."""
        self.grades_items = self._fetch_canvas_courses()
        self.grades_selected_index = 0
        self.grades_start_index = 0
        # Sidebar items for consistency with icon mapping
        sidebar_items = ["Main Page", "Grades", "Settings"]
        self.display.show_grades_menu(
            self.grades_items,
            self.grades_selected_index,
            title="Grades",
            nav_items=sidebar_items,
            nav_selected_index=1,
            start_index=self.grades_start_index,
            max_visible=5,
    ...
    def handle_grades_input(self, action):
        if action == 'up':
            if self.grades_items:
                self.grades_selected_index = max(0, self.grades_selected_index - 1)
                if self.grades_selected_index < self.grades_start_index:
                    self.grades_start_index = self.grades_selected_index
                self.show_grades_screen()
        elif action == 'down':
            if self.grades_items:
                self.grades_selected_index = min(len(self.grades_items) - 1, self.grades_selected_index + 1)
                # Scroll window if needed
                visible_max = self.grades_start_index + 5
                if self.grades_selected_index >= visible_max:
                    self.grades_start_index += 1
                self.show_grades_screen()
        elif action == 'left':
            self.current_screen = "main"
            self.selected_index = self.main_menu_items.index("Grades") if "Grades" in self.main_menu_items else 1
            self.show_main_menu()
        elif action == 'select' or action == 'right':
            # Future: open assignments for selected course
            pass
