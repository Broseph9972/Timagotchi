...
class FontManager:
    ...
    def load_fonts(self):
        ...
        try:
            if os.path.exists(self.fonts_file):
                ...
                # Create default font configuration if file doesn't exist
                self.fonts = self._get_default_fonts()
                self.current_font_name = 'DejaVu Sans'
                self.save_fonts()
            
            # Scan fonts directory for additional .ttf files
        ...
        try:
            ...
            if os.path.exists(fonts_path):
                ...
                for font_path in all_fonts:
                    ...
                    # Create a display name from filename (remove extension, titlecase)
                    font_name = os.path.splitext(font_filename)[0].replace('-', ' ').replace('_', ' ')
                    
                    # Don't override existing font definitions
                    if font_name not in self.fonts:
                        self.fonts[font_name] = {
                            'name': font_name,
                            'path': font_filename,
                            'regular': font_filename,
                            'bold': font_filename  # Use same file for bold if no specific bold variant
                        }
        except Exception as e:
    ...
    def get_font_path(self, style='regular'):
        ...
        # Try bundled fonts directory first
        ...
        system_paths = [
            ...
            "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",  # Ultimate fallback
        ]
        ...
        # If all else fails, return the bundled path (will trigger fallback in display code)
        return bundled_path
