...
# Tetris piece colors (RGB)
COLORS = [
    (0, 0, 0),        # Empty
    (120, 37, 179),   # Purple - I piece
    (100, 179, 179),  # Cyan - J piece
    (80, 134, 22),    # Green - L piece
    (180, 134, 22),   # Orange - O piece
    (180, 34, 22),    # Red - S piece
    (180, 34, 122),   # Pink - T piece
    (80, 80, 200),    # Blue - Z piece
]

# Tetris piece shapes (4x4 grid indices)
FIGURES = [
    [[1, 5, 9, 13], [4, 5, 6, 7]],                           # I
    [[4, 5, 9, 10], [2, 6, 5, 9]],                           # S
    [[6, 7, 9, 10], [1, 5, 6, 10]],                          # Z
...
class TetrisWaveshare:
    ...
    def __init__(self, display, input_handler):
        ...
        # Game board sizing for 128x128 display
        # Board: 10 wide x 20 tall
        # Each cell: 5x5 pixels = 50x100 pixels for board
        # Leave room for score on the right
        self.cell_size = 5
        self.board_x = 2  # Left margin
        self.board_y = 8  # Top margin (room for score)
        self.board_width = 10
        self.board_height = 20
        
        self.game = None
        self.last_drop_time = 0
        self.drop_interval = 0.5  # seconds between auto-drops
    ...
    def start(self):
        ...
        while self.running:
            ...
            # Check for exit keys (KEY1, KEY2, KEY3)
            if action in ('key1', 'key2', 'key3'):
                self.running = False
                return action  # Return which key was pressed for menu navigation
            ...
            if self.game.state == "playing":
                ...
                # Show game over, wait for input to restart or exit
                if action == 'select':
                    self.game = TetrisGame(self.board_height, self.board_width)
                    self.render()
    ...
    def render(self):
        ...
        if self.game.state == "gameover":
            # Semi-transparent dark overlay effect
    ...
    def _draw_cell(self, col, row, color):
        ...
        # Draw filled rectangle with slight border effect
    ...
    def _draw_next_preview(self):
        ...
        # Show the current piece type's first rotation as preview
        # (In a full implementation, you'd track the next piece)
        # For now just show a small hint area
        self.display.draw.rectangle(
